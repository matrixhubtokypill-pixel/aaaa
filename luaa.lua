...
-- ========================================
-- PERMANENT ANTI-FLING BYPASS (NEVER STOPS)
-- ========================================
local player = game.Players.LocalPlayer
local replicatedStorage = game:GetService("ReplicatedStorage")
local runService = game:GetService("RunService")

-- BLOCK CHECKER_4 PERMANENTLY
...
-- DELETE EVERY ANTI-FLING SCRIPT (RUNS CONSTANTLY)
...
local function keepAlive()
    pcall(function()
        ...
        if hum then
            -- Prevent PlatformStand (main cause of being stuck)
...
-- ===== RUN ON HEARTBEAT (EVERY FRAME - INSTANT RESPONSE) =====
runService.Heartbeat:Connect(function()
    pcall(function()
        ...
        -- Remove attachments instantly if they appear
        local root = char:FindFirstChild("HumanoidRootPart")
        if root then
            for _, child in pairs(root:GetChildren()) do
                if child:IsA("Attachment") or child:IsA("Weld") or child:IsA("Constraint") then
                    local name = string.lower(child.Name or "")
                    if string.find(name, "anti") or string.find(name, "fling") then
                        child:Destroy()
                    end
                end
            end
...
print("🔥 PERMANENT anti-fling bypass active!")
print("🔄 Re-applies every 0.5 seconds + every frame")
...
do
    ...
    function Script:GetClosestPlayerToCursor(Max, FOV, Feature)
        ...
        local Checks = getgenv().saved.Osiris['General']['Checks'][Feature] or 
                       getgenv().saved.Osiris['General']['Checks']['Silent Aim']
        ...
        for _, Player in ipairs(Players:GetPlayers()) do
            ...
            if Player and Player.Character then
                ...
                if Checks['Knocked'] and Player.Character and CurrentGame.Functions.IsKnocked(Player.Character) then
                    continue
                end

                if Checks['Self Knocked'] and CurrentGame.Functions.IsKnocked(Self.Character) then
                    continue
                end

                if Checks['Carried'] and CurrentGame.Functions.IsGrabbed(Player) then
                    continue
                end
