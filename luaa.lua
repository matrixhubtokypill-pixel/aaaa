    ...
    InitializeLocals()
    -- Add this after InitializeLocals() function
-- Add this after InitializeLocals() function
...
do
    ...
    function Script:UpdateStatusUI()
        -- Don't update if status is hidden
        if not getgenv().saved.Osiris['General']['Show Status'] then
            LogoText.Visible = false
            SilentStatus.Visible = false
            TriggerStatus.Visible = false
            return
        end
        
        -- Always keep visible if enabled
        LogoText.Visible = true
        SilentStatus.Visible = true
        TriggerStatus.Visible = true
        ...
        local newSilentColor = (silentEnabled and silentTarget) and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(255, 255, 255)
        local newTriggerColor = (triggerEnabled and triggerState and triggerTarget) and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(255, 255, 255)
        
        -- UPDATE ONLY IF CHANGED
        if newSilentName ~= cachedSilentName then
            SilentStatus.Text = string.format("Silent Aim: %s", newSilentName)
            cachedSilentName = newSilentName
        end
        
        if newTriggerName ~= cachedTriggerName then
            TriggerStatus.Text = string.format("Triggerbot: %s", newTriggerName)
            cachedTriggerName = newTriggerName
        end
        
        if newSilentColor ~= cachedSilentColor then
            SilentStatus.Color = newSilentColor
            cachedSilentColor = newSilentColor
        end
        
        if newTriggerColor ~= cachedTriggerColor then
            TriggerStatus.Color = newTriggerColor
            cachedTriggerColor = newTriggerColor
        end
        
        -- Update positions only when viewport changes (not every frame)
        updateCounter = updateCounter + 1
        if updateCounter >= 30 then  -- Update position every ~0.5 seconds
    ...
    do
        ...
        function Script:SilentAimFunc(Tool)
            ...
            else                if string.find(GameName, "Da Hood") then
...
function Script:ApplyGunDelay(tool)
    ...
    local tolerance = tool:FindFirstChild("ToleranceCooldown")
    if tolerance and tolerance:IsA("ValueBase") then
...
local function WatchCharacterDelays(char)
    if not char then return end
...
Self.CharacterAdded:Connect(WatchCharacterDelays)
...
task.spawn(function()
    while true do
        ...
        function Script:Triggerbot()
            ...
            if not selfCharacter or not selfCharacter:FindFirstChild("HumanoidRootPart") then
                TriggerPart.Position = Vector3.zero
                return
            end
            ...
            if targetingMode == 'Auto' then
                -- No Radius key in config — use huge screen FOV, world distance checked below
                Script.Locals.TriggerbotTarget = Script:GetClosestPlayerToCursor(
                    math.huge,
                    math.huge
                )
            end
            -- Toggle mode: target is set by keybind only
    ...
    do
        ...
        RBXConnection(UserInputService.InputBegan, function(Input, Processed)
            ...
            -- Silent Aim target toggling (only works in Toggle mode)
        ...
        -- Silent Aim targeting (Auto mode)
    ...
    local function GetPlayerESP(player)
        ...
        billboard.Size = UDim2.new(0, 48, 0, 22)
        ...
        healthBarBg.Size = UDim2.new(0.9, 0, 0, 3)
        healthBarBg.Position = UDim2.new(0.05, 0, 0, 11)
        ...
        armorBarBg.BackgroundColor3 = Color3.fromRGB(35, 35, 35)
        armorBarBg.BorderSizePixel = 1
        armorBarBg.BorderColor3 = Color3.fromRGB(0, 0, 0)
        armorBarBg.Size = UDim2.new(0.9, 0, 0, 3)
        armorBarBg.Position = UDim2.new(0.05, 0, 0, 16)
        armorBarBg.Parent = mainFrame
    ...
    local function GlobalESPUpdate()
        ...
        for _, plr in ipairs(Players:GetPlayers()) do
            ...
            if root and ESP_Cfg['Enabled'] then
                ...
                -- Bigger when close, stays small when far
                ...
                local showName = ESP_Cfg['Names']
                local showDist = ESP_Cfg['Distance']
                local display = ""
                if showName then
                    display = plr.DisplayName or plr.Name
                end
                ...
                local showHealth = ESP_Cfg['Health Bar']
                esp_data.HealthBarBg.Visible = showHealth
                if showHealth then
                ...
                local showArmor = ESP_Cfg['Armor Bar']
                esp_data.ArmorBarBg.Visible = showArmor
                if showArmor then
                    ...
                    if bodyEffects then
                        ...
                        if armorValue then
                            ...
                            esp_data.ArmorBar.Size = UDim2.new(0, 0, 1, 0)
                        end
                    else
                        esp_data.ArmorBar.Size = UDim2.new(0, 0, 1, 0)
                    end
...
RunService.Heartbeat:Connect(function(delta)
    ...
    if espCooldown >= 0.02 then -- 50 FPS (1/50 = 0.02)
        espCooldown = 0
        GlobalESPUpdate()
    end
end)
...
local AnimPacks = {
    ...
    Stylish = {
        ...
        Fall = "rbxassetid://616136790",
        Idle = "rbxassetid://616138447",
        Pose = "rbxassetid://616139451"
    },
    Werewolf = {
    ...
    Levitation = {
        ...
        Fall = "rbxassetid://2510197257",
        Idle = "rbxassetid://2510197830",
    ...
    Toy = {
        ...
        Fall = "rbxassetid://782841498",
        Idle = "rbxassetid://782846423",
        Pose = "rbxassetid://782846423"
    },
    Zombie = {
        ...
        Pose = "rbxassetid://10921345304"
    }
}

local PackNames = {"None", "Bubbly", "Stylish", "Werewolf", "Mage", "Levitation", "Ninja", "Pirate", "Cartoony", "Toy", "Zombie"}

-- FIX: Convert stored pack names to asset IDs
local function ConvertSavedAnimations()
    local anims = CONFIG.Animations
    if not anims then return end
    
    for animType, value in pairs(anims) do
        -- If value is a pack name (not an asset ID)
        if value and value ~= "" and not string.match(value, "rbxassetid://") then
            -- Look up the asset ID from AnimPacks
            local pack = AnimPacks[value]
            if pack then
                -- Get the correct animation type key
                local key = animType:lower()
                if key == "idle" then anims[key] = pack.Idle
                elseif key == "walk" then anims[key] = pack.Walk
                elseif key == "run" then anims[key] = pack.Run
                elseif key == "jump" then anims[key] = pack.Jump
                elseif key == "fall" then anims[key] = pack.Fall
                elseif key == "pose" then anims[key] = pack.Pose
                end
            end
        end
    end
...
-- Run conversion
ConvertSavedAnimations()

-- Track selected pack names for UI
if not getgenv().saved.Osiris.Player.Avatar.SelectedPacks then
    getgenv().saved.Osiris.Player.Avatar.SelectedPacks = {
        Idle = "None",
        Walk = "None",
        Run = "None",
        Jump = "None",
...
local SelectedPacks = getgenv().saved.Osiris.Player.Avatar.SelectedPacks

-- Helper to update config animations from pack name
local function SetAnimPack(animType, packName)
    local CONFIG = getgenv().saved.Osiris.Player.Avatar
    if not CONFIG.Animations then
        CONFIG.Animations = { idle = "", walk = "", run = "", jump = "", fall = "", pose = "" }
    end
    local pack = AnimPacks[packName] or AnimPacks.None
    local key = animType:lower()
    CONFIG.Animations[key] = pack[animType]
    SelectedPacks[animType] = packName
end

-- Try to detect current packs from saved animations
for animType, packName in pairs(SelectedPacks) do
    local savedId = CONFIG.Animations and CONFIG.Animations[animType:lower()]
    if savedId and savedId ~= "" then
        for pack, data in pairs(AnimPacks) do
            if data[animType] == savedId then
                SelectedPacks[animType] = pack
                break
            end
        end
    end
...
-- Convert pack names to asset IDs in the config
local function EnsureAssetIDs()
    local anims = CONFIG.Animations
    if not anims then return end
...
local function applyCustomAnimations(character)
    ...
    if animateScript then
        ...
        local poseFolder = animateScript:FindFirstChild("pose")
        ...
        if poseFolder and CONFIG.Animations.pose then
            setAnimId(poseFolder, "PoseAnim", CONFIG.Animations.pose)
        end
    end
end
...
local function applyAvatar(character)
    if not CONFIG.Enabled then 
        ...
        if humanoid then
            ...
            if defaultDesc then
                pcall(function() humanoid:ApplyDescription(defaultDesc) end)
            end
        end
        return 
    end
    ...
    if descSuccess and targetDesc then
        pcall(function() humanoid:ApplyDescription(targetDesc) end)
    end
...
-- UI Helper Functions
function GetCurrentPack(animType)
    local CONFIG = getgenv().saved.Osiris.Player.Avatar
    if not CONFIG or not CONFIG.Animations then return "None" end
    
    local currentId = CONFIG.Animations[animType:lower()]
    if currentId and currentId ~= "" then
        for packName, pack in pairs(AnimPacks) do
            if pack[animType] == currentId then
                return packName
            end
        end
    end
...
function SetAnimPackFromUI(animType, packName)
    SetAnimPack(animType, packName)
