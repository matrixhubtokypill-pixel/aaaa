...
-- FORCE SET TO 0 IMMEDIATELY FIRST
shotland.Value = 0
shotreseter.Value = 0
shottotal.Value = 0
warning.Value = 0
...
-- Then keep resetting when changed
...
-- CRITICAL: Reset ShotLand to prevent accuracy calculation
shotland:GetPropertyChangedSignal("Value"):Connect(function()
    shotland.Value = 0
end)

-- Force reset continuously for first 2 seconds
task.spawn(function()
    for i = 1, 20 do
        task.wait(0.1)
...
do
    ...
    function Script:GetClosestPlayerToCursor(Max, FOV)
        ...
        local Conditions = shared.ByteAxis.Config['General']['Conditions']
        ...
        for _, Player in ipairs(Players:GetPlayers()) do
            ...
            if Player and Player.Character then
                ...
                if Conditions['Force Field'] and Character:FindFirstChild("Forcefield") then
                    continue
                end
    ...
    if playerCharacter and playerCharacter:FindFirstChild("Humanoid") and playerCharacter.Humanoid:FindFirstChild("Animator") then
        ...
        task.delay(0.01, function()
            ...
            local function ShootGun(p34)
                ...
                if shared.ByteAxis.Config['Silent Aim']['Visual Redirection']['Enabled'] then
...
do
    ...
    function Script:GetHitPosition(Mode)
        if Mode == 'Assist' then
            local Config = shared.ByteAxis.Config['Aim Assist']
            ...
            if Config['Hit Part'] == 'Nearest Point' then
                local NearestPoint
                if Config['Nearest Point']['Mode'] == 'Smart' then
                    NearestPoint = Script:GetClosestPointOnPart(NearestPart, Config['Nearest Point']['Scale'])
                else
                    NearestPoint = Script:GetClosestPointOnPartBasic(NearestPart)
                end
                HitPosition = NearestPoint
        
            elseif Config['Hit Part'] == 'Nearest Part' then
        ...
        if Mode == 'Silent' then 
            local Config = shared.ByteAxis.Config['Silent Aim']
    ...
    function Script:UpdateBox()
        if Script.Locals.SilentAimTarget and Script.Locals.SilentAimTarget.Character then
            ...
            if (Object and Humanoid and RootPart) then		
                ...
                Script.Locals.FieldOfViewOne.Visible = (Visible and shared.ByteAxis.Config['Silent Aim']['FOV']['Mode'] == '2D Box' and shared.ByteAxis.Config['Silent Aim']['FOV']['Visible']) or false
    ...
    function Script:ShouldShoot(Target)
        ...
        if Conditions['Force Field'] and Target.Character:FindFirstChild("Forcefield") then
        ...
        if shared.ByteAxis.Config['Silent Aim']['FOV']['Mode'] == '2D Box' then
            RadiusX = Box.X
            RadiusY = Box.Y
        else
            RadiusX = CurrentFOV
        ...
        if shared.ByteAxis.Config['Silent Aim']['FOV']['Enabled'] and (shared.ByteAxis.Config['Silent Aim']['FOV']['Mode'] == '2D Box' or shared.ByteAxis.Config['Silent Aim']['FOV']['Mode'] == 'Circle') and not (RadiusX > DistanceX and RadiusY > DistanceY and (DistanceX^2 + DistanceY^2) < (1/0)^2) then
            allConditionsPassed = false
        end
    ...
    function Script:SilentAimFunc(Tool)
        if (string.find(GameName, "Dee Hood") or string.find(GameName, "Der Hood")) and shared.ByteAxis.Config['Silent Aim']['Enabled'] then
            if Script.Locals.SilentAimTarget and Script.Locals.SilentAimTarget.Character then
            ...
            if string.find(GameName, "Da Hood") then
                    ...
                    if GunType == "Shotgun" then
                        if Check and (NoClueWhatThisIs.Value >= 1 and (not _G.GUN_COMBAT_TOGGLE and DaHood.CanShoot(Self.Character))) then
                            ...
                            for _ = 1, 5 do
                                ...
                                if shared.ByteAxis.Config['Weapons']['Spread Modifier']['Enabled'] then
                                    local spreadData = shared.ByteAxis.Config['Weapons']['Spread Modifier'][Tool.Name]
                                    local spreadReduction = spreadData and spreadData['Value'] or 1
                                    local randomizer = shared.ByteAxis.Config['Weapons']['Spread Modifier']['Randomizer']
                ...
                if shared.ByteAxis.Config['Silent Aim']['Enabled'] and Script.Locals.SilentAimTarget and Script.Locals.SilentAimTarget.Character then
    ...
    local Enabled = shared.ByteAxis.Config['Aim Assist']['Enabled'] 
    local Cond = shared.ByteAxis.Config['General']['Conditions']
    if (Enabled and Script.Locals.AimAssistTarget and Script.Locals.AimAssistTarget.Character) then
        ...
        if Cond['Force Field'] and Character:FindFirstChild("Forcefield") then return end
        ...
        local Smoothing = Config['Snappiness']
        if Config['Strength']['Enabled'] then
            ...
            if velocityMagnitude > 15 then  
                Smoothing = Config['Strength']['Moving']
            else 
                Smoothing = Config['Strength']['Base']
            end
        end
    ...
    function Script:Physics()
        ...
        if shared.ByteAxis.Config['Player']['Anti Fall'] then
            if Hum.Health > 1 and Hum:GetState() == Enum.HumanoidStateType.FallingDown then
                Hum:ChangeState("GettingUp")
            end
        end

        if Script.Locals.IsWalkSpeeding and shared.ByteAxis.Config['Player']['Walk Speed']['Enabled'] then
            Hum.WalkSpeed = shared.ByteAxis.Config['Player']['Walk Speed']['Speed']
        end
    end
...
do
    local FOVConfig = shared.ByteAxis.Config['Silent Aim']['FOV']
    local SilentAimConfig = shared.ByteAxis.Config['Silent Aim']
    ...
    local Configuration = shared.ByteAxis.Config['Silent Aim']['FOV']['Configuration']
    local function UpdateDrawings()
        local Character = Self.Character
        if not Character then return end
        local Tool = Character:FindFirstChildWhichIsA("Tool")
        
        if Configuration['Enabled'] and Tool then
            if table.find(WeaponInfo.Shotguns, Tool.Name) then
                CurrentFOV = Configuration['Shotguns']['Circle']
                CurrentFOVX = Configuration['Shotguns']['2D Box']['X']
                CurrentFOVY = Configuration['Shotguns']['2D Box']['Y']
            elseif table.find(WeaponInfo.Pistols, Tool.Name) then
                CurrentFOV = Configuration['Pistols']['Circle']
                CurrentFOVX = Configuration['Pistols']['2D Box']['X']
                CurrentFOVY = Configuration['Pistols']['2D Box']['Y']
            else
                CurrentFOV = Configuration['Automatics']['Circle']
                CurrentFOVX = Configuration['Automatics']['2D Box']['X']
                CurrentFOVY = Configuration['Automatics']['2D Box']['Y']
            end
        else
            CurrentFOV = Configuration['Automatics']['Circle']
            CurrentFOVX = Configuration['Automatics']['2D Box']['X']
        ...
        Script.Locals.FieldOfViewTwo.Visible = shared.ByteAxis.Config['Silent Aim']['FOV']['Mode'] == 'Circle' and shared.ByteAxis.Config['Silent Aim']['FOV']['Visible']
    ...
    RBXConnection(UserInputService.InputBegan, function(Input, Processed)
        local AimAssistKey = Enum.KeyCode[shared.ByteAxis.Config['General']['Keybind']['Aim Assist']:upper()]
        local SilentAimTarget = Enum.KeyCode[shared.ByteAxis.Config['General']['Keybind']['Silent Aim Target']:upper()]
        local ESPKey = Enum.KeyCode[shared.ByteAxis.Config['General']['Keybind']['ESP']:upper()]
        
        local WSKey = Enum.KeyCode[shared.ByteAxis.Config['General']['Keybind']['Walk Speed']:upper()]
        ...
        if Input.KeyCode == SilentAimTarget and SilentAimConfig['Mode'] == 'Target' then
            ...
            if SP then 
                Script.Locals.SilentAimTarget = Script:GetClosestPlayerToCursor(
                    SilentAimConfig['Max Distance'] * 102220,
        ...
        if Input.KeyCode == ESPKey then
            shared.ByteAxis.Config['Player']['ESP']['Enabled'] = not shared.ByteAxis.Config['Player']['ESP']['Enabled']
        end
        ...
        if Input.KeyCode == AimAssistKey then
            ...
            if SP2 then
                Script.Locals.AimAssistTarget = Script:GetClosestPlayerToCursor(
                    SilentAimConfig['Max Distance'] * 700,
                    math.huge
                )
            else
...
local function GetTracer()
    ...
    TracerLine = Script.Visuals.new("Line")
...
RBXConnection(RunService.PreRender, LPH_NO_VIRTUALIZE(function()
    ...
    local tracerEnabled = shared.ByteAxis.Config['Silent Aim']['Tracer']
    local target = Script.Locals.SilentAimTarget
        ...
        if rootPart and humanoid and humanoid.Health > 0 then
            ...
            if onScreen then
                ...
                if fovConfig['Enabled'] then
                    local distX = math.abs(screenPos.X - mousePos.X)
                    local distY = math.abs(screenPos.Y - mousePos.Y)
                    local sizeX, sizeY
                    
                    if fovConfig['Mode'] == 'Circle' then
                        sizeX = CurrentFOV
                        sizeY = CurrentFOV
                        insideFOV = (distX * distX + distY * distY) <= (CurrentFOV * CurrentFOV)
                    elseif fovConfig['Mode'] == '2D Box' then
                        local config = fovConfig['Configuration']
                        local tool = Self.Character and Self.Character:FindFirstChildWhichIsA("Tool")
                        
                        if config['Enabled'] and tool then
                            if table.find(WeaponInfo.Shotguns, tool.Name) then
                                sizeX = config['Shotguns']['2D Box']['X']
                                sizeY = config['Shotguns']['2D Box']['Y']
                            elseif table.find(WeaponInfo.Pistols, tool.Name) then
                                sizeX = config['Pistols']['2D Box']['X']
                                sizeY = config['Pistols']['2D Box']['Y']
                            else
                                sizeX = config['Automatics']['2D Box']['X']
                                sizeY = config['Automatics']['2D Box']['Y']
                            end
                        else
                            sizeX = config['Automatics']['2D Box']['X']
                            sizeY = config['Automatics']['2D Box']['Y']
                        ...
                        local rootSize = rootPart.Size.Y
                        local scaleFactor = (rootSize * Camera.ViewportSize.Y) / (screenPos.Z * 2) * 80 / Camera.FieldOfView
                        local w = sizeX * scaleFactor
                        local h = sizeY * scaleFactor
                        
                        insideFOV = distX <= (w / 2) and distY <= (h / 2)
                    end
                else
                    insideFOV = true
                end
                ...
                if insideFOV then
                    tracer.Color = shared.ByteAxis.Config['Player']['ESP']['Targeted Color']
...
local function GetPlayerESP(player)
    ...
    text.TextSize = 13
    text.TextColor3 = shared.ByteAxis.Config['Player']['ESP']['Normal Color']
...
local function GlobalESPUpdate()
    local ESP_Cfg = shared.ByteAxis.Config['Player']['ESP']
    ClearDeadESP()
...
RunService.Heartbeat:Connect(function()
    if shared.ByteAxis.Config['Player']['Headless'] and Self.Character then
...
local WallHopEnabled = shared.ByteAxis.Config['Player']['Wall Hop']
...
local function checkForWallHop()
    ...
    for _, dir in ipairs(directions) do
        local result = workspace:Raycast(origin, dir * WallHopConfig.TouchDistance, hopRayParams)
        if result then
            local normal = result.Normal
            if math.abs(normal.Y) < WallHopConfig.WallNormalThreshold then
                return true, normal
            end
        end
    end
...
local function performWallHop()
    ...
    if tick() - lastWallHopTime < WallHopConfig.CooldownTime then return end
    ...
    root.AssemblyLinearVelocity = Vector3.new(
        currentVel.X + (awayDir.X * WallHopConfig.WallJumpAwayBoost),
        WallHopConfig.WallJumpUpBoost,
        currentVel.Z + (awayDir.Z * WallHopConfig.WallJumpAwayBoost)
    )
    
    canWallHop = false
    lastWallHopTime = tick()
    
    task.wait(WallHopConfig.CooldownTime)
    canWallHop = true
end
